		...
		<div class="product-card-actions">
			...
			if ( $product->is_purchasable() && $product->is_in_stock() && ! $product->is_type( 'variable' ) ) {
				...
				echo apply_filters(
					...
					sprintf(
						'<a href="%s" data-quantity="1" class="%s" %s><span class="pimou-cart-button-icon" aria-hidden="true"></span><span>%s</span></a>',
						esc_url( $product->add_to_cart_url() ),
						esc_attr( implode( ' ', array_filter( array( 'pimou-btn', 'pimou-btn-outline', 'add_to_cart_button', 'ajax_add_to_cart' ) ) ) ),
				...
				echo '<a class="pimou-btn pimou-btn-primary" href="' . esc_url( $buy_now_url ) . '" rel="nofollow">' . esc_html__( 'Mua ngay', 'pimou-theme' ) . '</a>';
			} else {
				echo '<a class="pimou-btn pimou-btn-outline" href="' . esc_url( get_permalink( $product_id ) ) . '"><span class="pimou-cart-button-icon" aria-hidden="true"></span><span>' . esc_html__( 'Giỏ hàng', 'pimou-theme' ) . '</span></a>';
				echo '<a class="pimou-btn pimou-btn-primary" href="' . esc_url( get_permalink( $product_id ) ) . '">' . esc_html__( 'Mua ngay', 'pimou-theme' ) . '</a>';
			}
			?>
		</div>
	</div>
